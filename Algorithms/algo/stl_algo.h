...
namespace MiniSTL {
    ...
    typename iterator_traits<InputIterator>::difference_type count(InputIterator first, InputIterator last, T value) {
    ...
    template <class InputIterator, class T> InputIterator find(InputIterator first, InputIterator last, T value) {
    ...
    ForwardIterator1 __find_end(ForwardIterator1 first1, ForwardIterator1 last1, ForwardIterator2 first2,
                                ForwardIterator2 last2, forward_iterator_tag, forward_iterator_tag) {
        if (first2 == last2)
            return last1;
        else {
    ...
    BidirectionalIterator1 __find_end(BidirectionalIterator1 first1, BidirectionalIterator1 last1,
                                      BidirectionalIterator1 first2, BidirectionalIterator1 last2,
    ...
    ForwardIterator remove(ForwardIterator first, ForwardIterator last, T value) {
        ...
        ForwardIterator next = first;
        return first == last ? first : remove_copy(++next, last,
    ...
    void __rotate(BidrectionalIterator first, BidrectionalIterator middle, BidrectionalIterator last, Distance,
    ...
    void __rotate(RandomAccesslIterator first, RandomAccesslIterator middle, RandomAccesslIterator last, Distance,
    ...
    // gcd:求取最大公约数（效率不如减损术）
    template <class EuclideanRingElement> EuclideanRingElement __gcd(EuclideanRingElement m, EuclideanRingElement n) {
        while (n != 0) {
    ...
    void __rotate(RandomAccesslIterator first, RandomAccesslIterator last, RandomAccesslIterator initial,
                  Distance shift, T *) {
        T value = *initial;
    ...
    OutputIterator transform(InputIterator1 first1, InputIterator1 last1, InputIterator1 first2, OutputIterator result,
    ...
    template <class BidirectionIterator> bool next_permutation(BidirectionIterator first, BidirectionIterator last) {
        //以下为边界判定
        if (first == last)
            return false;
    ...
    template <class BidirectionIterator> bool pre_permutation(BidirectionIterator first, BidirectionIterator last) {
        //以下为边界判定
        if (first == last)
            return false;
        BidirectionIterator i = first;
    ...
    // insertion_sort:插入排序
    template <class RandomAccessIterator> void __insertion_sort(RandomAccessIterator first, RandomAccessIterator last) {
        if (first == last)
            return;
    ...
    template <class RandomAccessIterator, class T> void __unguarded_linear_insert(RandomAccessIterator last, T value) {
    ...
    // median:求取三点中值
    template <class T> inline const T &__median(const T &a, const T &b, const T &c) {
        if (a < b)
            if (b < c)
                return b; // a<b<c
            else if (a < c)
    ...
    void __introsort_loop(RandomAccessIterator first, RandomAccessIterator last, T *, Size depth_limit) {
        ...
        while (last - first > /*__stl_threshold*/ 16) {
            ...
            RandomAccessIterator cut =
                __unguarded_partition(first, last, T(__median(*first, *(first + (last - first) / 2), *(last - 1))));
    ...
    */
    ...
    template <class RandomAccessIterator, class T, class Distance>
    inline pair<RandomAccessIterator, RandomAccessIterator> __equal_range(RandomAccessIterator first,
    ...
    template <class ForwardIterator, class T, class Distance>
    inline pair<ForwardIterator, ForwardIterator> __equal_range(ForwardIterator first, ForwardIterator last,
    ...
    void __merge_adaptive(BidirectionalIterator first, BidirectionalIterator middle, BidirectionalIterator last,
        ...
        if (len1 <= len2 && len1 <= buffer.size()) {
            ...
            BidirectionalIterator new_middle =
                __rotate_adaptive(first_cut, middle, second_cut, len1 - len11, len22, buffer, buffer, buffer_size);
            //针对左端递归
