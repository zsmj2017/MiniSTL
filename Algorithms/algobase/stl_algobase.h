...
namespace MiniSTL {
    ...
    template <class InputIterator, class OutputIterator> struct __copy_dispatch { // 仿函数对象
    ...
    // 偏特化处理
    template <class T> struct __copy_dispatch<T *, T *> {
    ...
    // 偏特化处理
    template <class T> struct __copy_dispatch<const T *, T *> {
    ...
    template <class T> inline T *__copy_t(const T *first, const T *last, T *result, _true_type) {
        memmove(result, first, sizeof(T) * (last - first));
        return result + (last - first);
    }

    // 原始指针是一种random_access_iterator
    template <class T> inline T *__copy_t(const T *first, const T *last, T *result, _false_type) {
    ...
    inline BidirectionalIter2 __copy_backward(RandomAccessIter first, RandomAccessIter last, BidirectionalIter2 result,
    ...
    template <class BidirectionalIter1, class BidirectionalIter2, class BoolType> struct __copy_backward_dispatch {
        BidirectionalIter2 operator()(BidirectionalIter1 first, BidirectionalIter1 last, BidirectionalIter2 result) {
            return __copy_backward(first, last, result, iterator_category_t<BidirectionalIter1>(),
                                   difference_type_t<BidirectionalIter1>());
        }
    };

    template <class T> struct __copy_backward_dispatch<T *, T *, _true_type> {
        T *operator()(const T *first, const T *last, T *result) {
    ...
    template <class T> struct __copy_backward_dispatch<const T *, T *, _true_type> {
        T *operator()(const T *first, const T *last, T *result) {
            return __copy_backward_dispatch<T *, T *, _true_type>()(first, last, result);
        }
    };

    template <class BI1, class BI2> inline BI2 copy_backward(BI1 first, BI1 last, BI2 result) {
