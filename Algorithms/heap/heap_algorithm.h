...
inline void push_heap(RandomAccessIterator first, RandomAccessIterator last, const Compare& comp=Compare()) {
	...
	push_heap_aux(first, Distance((last - first) - 1), Distance(0), T(*(last - 1)),comp);
}

template <class RandomAccessIterator, class Distance, class T, class Compare>
void push_heap_aux(RandomAccessIterator first, Distance holeIndex, Distance topIndex, T value, const Compare& comp = Compare()) {
...
inline void pop_heap_aux(RandomAccessIterator first, RandomAccessIterator last, RandomAccessIterator result, T value, const Compare& comp = Compare()) {
	using Distance = difference_type_t<RandomAccessIterator>;
...
template <class RandomAccessIterator, class Distance, class T, class Compare>
void adjust_heap(RandomAccessIterator first, Distance holeIndex, Distance len, T value, const Compare& comp = Compare()) {
