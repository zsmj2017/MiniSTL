...
namespace MiniSTL {
    ...
    template <class Key, class T, class Compare = less<Key>, class Alloc = simpleAlloc<T>> class map {
        // friend declarations
    ...
    public: // ctor
        ...
        template <class InputIterator> map(InputIterator first, InputIterator last) : t(key_compare()) {
            t.insert_unique(first, last);
        }
        template <class InputIterator> map(InputIterator first, InputIterator last, const key_compare &comp) : t(comp) {
            t.insert_unique(first, last);
        }
    ...
    public: // swap
        //调用rb-tree接口
        void swap(map<Key, T, Compare, Alloc> &x) noexcept {
            t.swap(x.t);
        }
