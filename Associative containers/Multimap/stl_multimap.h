...
friend bool operator==(const multimap<_Key, _T, _Compare, _Alloc>& lhs, const multimap<_Key, _T, _Compare, _Alloc>& rhs);
template <class _Key, class _T, class _Compare, class _Alloc>
friend bool operator< (const multimap<_Key, _T, _Compare, _Alloc>& lhs, const multimap<_Key, _T, _Compare, _Alloc>& rhs);
...
public: // find
	...
	pair<iterator, iterator> equal_range(const key_type& x) noexcept { return t.equal_range(x); }
	pair<const_iterator, const_iterator> equal_range(const key_type& x) const noexcept {return t.equal_range(x); }
};
