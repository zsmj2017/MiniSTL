...
protected://Internal alias declarations
	...
	using __rb_tree_node = __rb_tree_node<Value>;
...
private:
	...
	iterator __rb_tree_find_key (const key_type& k);
...
rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__insert(base_ptr x_, base_ptr y_, const value_type & value){
	...
	left(z) = nullptr;
	right(z) = nullptr;
	__rb_tree_rebalance(z, header->parent);//重新调整红黑树（新增节点颜色在其中调整）
	++node_count;
	return iterator(z);
}

template<class Key, class Value, class KeyOfValue, class Compare, class Alloc>
inline void rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__rb_tree_rebalance(base_ptr x, base_ptr& root) {
	x->color = __rb_tree_red;
	while (x != root && x->parent->color == __rb_tree_red) {//当前父节点为红
		if (x->parent == x->parent->parent->left) {//父亲为爷爷的左子
			base_ptr y = x->parent->parent->right;//伯父
			if (y && y->color == __rb_tree_red) {//伯父存在且为红色
	...
	root->color = __rb_tree_black;//root永远为黑色
}
...
//右旋与左旋相对称
template<class Key, class Value, class KeyOfValue, class Compare, class Alloc>
inline void rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__rb_tree_rotate_right(base_ptr x, base_ptr & root){
	base_ptr y = x->left;
	x->left = y->right;
	if (y->right)
		x->right->parent = x;
...
template<class Key, class Value, class KeyOfValue, class Compare, class Alloc>
inline typename typename rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::iterator
rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::__rb_tree_find_key(const key_type & k){//为何设定const属性后find无法识别key_type为类型？
...
rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::insert_equal(const value_type & value){
	...
	return __insert(x, y, value);//x为新值插入点，y为其父
}

template<class Key, class Value, class KeyOfValue, class Compare, class Alloc>
inline typename rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::iterator
rb_tree<Key, Value, KeyOfValue, Compare, Alloc>::find(const value_type & value){
	return __rb_tree_find_key(KeyOfValue()(value));
}
