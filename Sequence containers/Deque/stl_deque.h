...
namespace MiniSTL {
    ...
    template <class T, class Alloc = simpleAlloc<T>> class deque {
    ...
    private: // aux_interface for ctor
        ...
        template <class Integer> void initialize_dispatch(Integer n, Integer val, _true_type) {
            initialize_map(static_cast<size_type>(n));
            fill_initialize(static_cast<value_type>(val));
        }
        template <class InputIterator> void initialize_dispatch(InputIterator first, InputIterator last, _false_type) {
    ...
    public: // ctor && dtor
        ...
        deque(size_type n, const value_type &val) : start(), finish(), map(nullptr), map_size(0) {
            initialize_map(n);
            fill_initialize(val);
        }
    ...
    private: // aux_interface for assign
        ...
        template <class Integer> void assign_dispatch(Integer n, Integer val, _true_type) {
            fill_assign(static_cast<size_type>(n), static_cast<value_type>(val));
        }
        template <class InputIterator> void assign_dispatch(InputIterator first, InputIterator last, _false_type) {
            assign_aux(first, last, iterator_category_t<InputIterator>());
        }
        template <class InputIterator> void assign_aux(InputIterator, InputIterator, input_iterator_tag);
        template <class ForwardIterator> void assign_aux(ForwardIterator, ForwardIterator, forward_iterator_tag);
    ...
    private: // aux_interface for insert
        ...
        template <class Integer> void insert_dispatch(iterator pos, Integer n, Integer val, _true_type) {
        ...
        template <class ForwardIterator> void insert_aux(iterator, ForwardIterator, ForwardIterator, size_type);
    ...
    template <class T, class Alloc> void deque<T, Alloc>::create_nodes(map_pointer nstart, map_pointer nfinish) {
        map_pointer cur;
        try {
            // 为每一个节点配置空间
    ...
    template <class T, class Alloc> void deque<T, Alloc>::destroy_nodes(map_pointer nstart, map_pointer nfinish) {
        for (map_pointer n = nstart; n < nfinish; ++n)
            deallocate_node(*n);
    }

    template <class T, class Alloc> void deque<T, Alloc>::initialize_map(size_type n) {
        // 所需节点数（整除则多配置一个）
    ...
    template <class T, class Alloc> void deque<T, Alloc>::fill_initialize(const value_type &val) {
        map_pointer cur;
        try {
            // 为每个缓冲区设定初值
    ...
    template <class T, class Alloc> inline deque<T, Alloc> &deque<T, Alloc>::operator=(const deque &rhs) {
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::reserve_map_at_back(size_type nodes_to_add) {
        // map_size-(finish.node-map+1) == 后端剩余node个数
        if (nodes_to_add + 1 > map_size - (finish.node - map))
            reallocate_map(nodes_to_add, false);
    }

    template <class T, class Alloc> inline void deque<T, Alloc>::reserve_map_at_front(size_type nodes_to_add) {
    ...
    template <class T, class Alloc>
    inline typename deque<T, Alloc>::iterator deque<T, Alloc>::reserve_elements_at_back(size_type n) {
    ...
    template <class T, class Alloc> void deque<T, Alloc>::new_elements_at_front(size_type new_elems) {
    ...
    template <class T, class Alloc> void deque<T, Alloc>::new_elements_at_back(size_type new_elems) {
        size_type new_nodes = (new_elems + buffer_size() - 1) / buffer_size();
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::push_back_aux(const value_type &value) {
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::push_front_aux(const value_type &value) {
        value_type value_copy = value;
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::pop_back_aux() {
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::pop_front_aux() {
        destroy(start.cur);
    ...
    template <class T, class Alloc>
    typename deque<T, Alloc>::iterator deque<T, Alloc>::insert_aux(iterator pos, const value_type &val) {
    ...
    template <class T, class Alloc> void deque<T, Alloc>::insert_aux(iterator pos, size_type n, const value_type &val) {
        ...
        if (elems_before < static_cast<difference_type>(length / 2)) {
            ...
            try {
                if (elems_after >= static_cast<difference_type>(n)) {
                    ...
                    MiniSTL::uninitialized_fill_copy(finish, pos + static_cast<difference_type>(n), value_copy, pos,
                                                     finish); // extensions
                    finish = new_finish;
    ...
    template <class T, class Alloc>
    template <class ForwardIterator>
    void deque<T, Alloc>::range_insert_aux(iterator pos, ForwardIterator first, ForwardIterator last,
    ...
    template <class T, class Alloc> inline deque<T, Alloc>::deque(deque &&rhs) {
        initialize_map(0);
        if (rhs.map) {
            swap(rhs);
        }
    ...
    template <class T, class Alloc> deque<T, Alloc> &deque<T, Alloc>::operator=(deque &&rhs) noexcept {
        clear();
        swap(rhs);
        return *this;
    }

    template <class T, class Alloc> inline deque<T, Alloc>::~deque() {
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::push_back(const value_type &value) {
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::push_front(const value_type &value) {
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::pop_back() {
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::pop_front() {
    ...
    template <class T, class Alloc> inline void deque<T, Alloc>::clear() {
    ...
    template <class T, class Alloc> typename deque<T, Alloc>::iterator deque<T, Alloc>::erase(iterator pos) {
        iterator next = pos + 1;
    ...
    template <class T, class Alloc> void deque<T, Alloc>::fill_assign(size_type n, const value_type &val) {
        if (n > size()) {
    ...
    template <class T, class Alloc> void deque<T, Alloc>::resize(size_type new_size, const value_type &val) {
        const size_type len = size();
        if (new_size < len)
    ...
    template <class T, class Alloc> void deque<T, Alloc>::swap(deque &rhs) noexcept {
    ...
    template <class T, class Alloc> inline bool operator==(const deque<T, Alloc> &lhs, const deque<T, Alloc> &rhs) {
    ...
    template <class T, class Alloc> inline void swap(const deque<T, Alloc> &lhs, const deque<T, Alloc> &rhs) {
        lhs.swap(rhs);
    }
