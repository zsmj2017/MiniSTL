...
protected://Internal member function
	...
	void reverse_map_at_front(size_type nodes_to_add);
...
inline void deque<T, Alloc, BufSiz>::create_map_and_nodes(size_type n){
	//所需节点数（整除则多配置一个）
	...
	finish.cur = finish.first + n % iterator::buffer_size();//若n%buffer_size==0,会多配置一个节点，此时cur指向该节点头部
}
...
inline void deque<T, Alloc, BufSiz>::reallocate_map(size_type nodes_to_add, bool add_at_front){
	...
	if (map_size > 2 * new_num_nodes) {
		...
		//规定新的nstart，若添加在前端则向后多移动n个单位(何解？）
...
template<class T, class Alloc, size_t BufSiz>
inline void deque<T, Alloc, BufSiz>::reverse_map_at_front(size_type nodes_to_add = 1){
...
template<class T, class Alloc, size_t BufSiz>
inline auto deque<T, Alloc, BufSiz>::insert_aux(iterator pos, const value_type & value){
...
template<class T, class Alloc, size_t BufSiz>
inline auto deque<T, Alloc, BufSiz>::erase(iterator pos){
	iterator next = pos + 1;
...
template<class T, class Alloc, size_t BufSiz>
inline auto deque<T, Alloc, BufSiz>::erase(iterator first,iterator last) {
...
template<class T, class Alloc, size_t BufSiz>
inline auto deque<T, Alloc, BufSiz>::insert(iterator pos, const value_type& value){
