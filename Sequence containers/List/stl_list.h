...
namespace MiniSTL {
    ...
    template <class T, class Alloc = simpleAlloc<T>> class list {
    ...
    private: // aux interface for assign
        ...
        template <class InputIterator> void assign_dispatch(InputIterator, InputIterator, _false_type);
    ...
    private: // aux interface for insert
        ...
        template <class InputIterator> void insert_dispatch(iterator, InputIterator, InputIterator, _false_type);
    ...
    template <class T, class Alloc> inline void list<T, Alloc>::empety_initialized() {
        node = get_node();
        node->next = node;
        node->prev = node;
    }

    template <class T, class Alloc> void list<T, Alloc>::fill_assign(size_type n, const value_type &val) {
        iterator i = begin();
        for (; i != end(); ++i, --n)
            *i = val;
        if (n > 0)
    ...
    template <class T, class Alloc> inline void list<T, Alloc>::resize(size_type new_size, const value_type &val) {
        iterator cur = begin();
        size_type len = 0;
    ...
    template <class T, class Alloc> inline typename list<T, Alloc>::iterator list<T, Alloc>::erase(iterator position) {
    ...
    template <class T, class Alloc> list<T, Alloc>::list(size_type n, const value_type &val) {
        empety_initialized();
        fill_insert(begin(), n, val);
    }

    template <class T, class Alloc> list<T, Alloc>::list(const list &rhs) {
        empety_initialized();
        insert(begin(), rhs.begin(), rhs.end());
    }

    template <class T, class Alloc> inline list<T, Alloc> &list<T, Alloc>::operator=(const list &rhs) noexcept {
        list temp(rhs);
        swap(temp);
        return *this;
    }
    ...
    template <class T, class Alloc> inline void list<T, Alloc>::splice(iterator position, list &, iterator i) {
        iterator j = i;
        ++j;
        // i==pos 自身无法插于自身之前
        // j==pos 已处于pos之前
    ...
    template <class T, class Alloc> void list<T, Alloc>::merge(list &x) {
    ...
    template <class T> inline void swap(list<T> &lhs, list<T> &rhs) {
        lhs.swap(rhs);
    }
    ...
    template <class T> inline bool operator!=(const list<T> &lhs, const list<T> &rhs) {
        return !(lhs == rhs);
    }
