...
namespace MiniSTL {
    ...
    template <class T, class Alloc = simpleAlloc<T>> class slist {
    ...
    public: // ctor && dtor
        ...
        slist(size_type n, const value_type &val) {
            empety_initialized();
            insert_after_fill(&head, n, val);
        }
        slist(std::initializer_list<value_type> ils) {
            empety_initialized();
            insert_after_range(&head, ils.begin(), ils.end());
        }
        // Here need not any dispatching tricks here, because insert_after_range
        // already does them.
        template <class InputIterator> slist(InputIterator first, InputIterator last) {
            empety_initialized();
    ...
    public: // copy operation
        slist(const slist &rhs) {
            empety_initialized();
            insert_after_range(&head, rhs.begin(), rhs.end());
        }
    ...
    public: // setter
        iterator before_begin() {
            return iterator(reinterpret_cast<list_node *>(&head));
        }
        iterator begin() {
            return iterator(reinterpret_cast<list_node *>(head.next));
        }
        iterator end() {
            return iterator(nullptr);
        }
        reference front() {
            return reinterpret_cast<list_node *>(head.next)->data;
        }
    ...
    public: // getter
        ...
        const_iterator begin() const noexcept {
            return const_iterator(reinterpret_cast<list_node *>(head.next));
        }
    ...
    private: // aux interface for insert
        ...
        template <class Integer> void insert_after_range_aux(list_node_base *pos, Integer n, Integer val, _true_type) {
    ...
    public: // assign
        ...
        slist &operator=(std::initializer_list<value_type> ils) {
            assign(ils);
            return *this;
        }
    ...
    public: // splice
        ...
        // insert all elements in rhs after pos in constant time, rhs must not be
        // *this
        ...
        void splice(iterator pos, slist &rhs) {
            if (rhs.head.next)
                slist_splice_after(slist_previous(&head, pos.node), &rhs.head, slist_previous(&rhs.head, nullptr));
        }
        // linear in distance(begin(),pos) and linerar in distance(rhs.begin(),i)
        void splice(iterator pos, slist &rhs, iterator i) {
            slist_splice_after(slist_previous(&head, pos.node), slist_previous(&rhs.head, i.node), i.node);
        }
        // linear in distance(begin(),pos),in distance(rhs.begin(),i) and in
        // distance(first,last)
        void splice(iterator pos, slist &rhs, iterator first, iterator last) {
            if (first != last)
                slist_splice_after(slist_previous(&head, pos.node), slist_previous(&rhs.head, first.node),
    ...
    template <class T, class Alloc> slist<T, Alloc> &slist<T, Alloc>::operator=(const slist &rhs) {
        if (this != &rhs) {
    ...
    template <class T, class Alloc> void slist<T, Alloc>::fill_assign(size_type n, const value_type &val) {
    ...
    template <class T, class Alloc> void slist<T, Alloc>::resize(size_type len, const value_type &val) {
    ...
    template <class T, class Alloc> void slist<T, Alloc>::remove(const value_type &val) {
        list_node_base *cur = &head;
        while (cur && cur->next) {
    ...
    template <class T, class Alloc> template <class Predicate> void slist<T, Alloc>::remove_if(Predicate pred) {
        list_node_base *cur = &head;
        while (cur && cur->next) {
    ...
    template <class T, class Alloc> void slist<T, Alloc>::unique() {
        list_node_base *cur = head.next;
        if (cur) {
            while (cur->next) {
                if (reinterpret_cast<list_node *>(cur)->data == reinterpret_cast<list_node *>(cur->next)->data)
                    erase_after(cur);
                else
                    cur = cur->next;
            }
    ...
    void slist<T, Alloc>::unique(BinaryPredicate pred) {
        ...
        if (cur) {
            while (cur->next) {
                if (pred(reinterpret_cast<list_node *>(cur)->data, reinterpret_cast<list_node *>(cur->next)->data))
                    erase_after(cur);
                else
                    cur = cur->next;
            }
    ...
    template <class T, class Alloc> void slist<T, Alloc>::merge(slist &rhs) {
        list_node_base *n1 = &head;
        while (n1->next && rhs.head.next) {
            if (reinterpret_cast<list_node *>(rhs.head.next)->data < reinterpret_cast<list_node *>(n1->next)->data)
    ...
    void slist<T, Alloc>::merge(slist &rhs, StrictWeakOrdering comp) {
        list_node_base *n1 = &head;
        while (n1->next && rhs.head.next) {
            if (comp(reinterpret_cast<list_node *>(rhs.head.next)->data, reinterpret_cast<list_node *>(n1->next)->data))
                slist_splice_after(n1, &rhs.head, rhs.head.next);
            n1 = n1->next;
        }
        if (rhs.head.next) {
    ...
    template <class T, class Alloc> void slist<T, Alloc>::sort() {
        // use merge sort
    ...
    // operator
    template <class T, class Alloc> inline bool operator==(const slist<T, Alloc> &lhs, const slist<T, Alloc> &rhs) {
    ...
    // swap
    template <class T, class Alloc> inline void swap(slist<T, Alloc> &lhs, slist<T, Alloc> &rhs) {
        lhs.swap(rhs);
    }

    // specialization of insert_iterator so that insertions will be constant rather
    // than linear time
    template <class T, class Alloc> class insert_iterator<slist<T, Alloc>> {
    private: // data member
