...
class slist {
 ...
 private:// aux interface for ctor
  void empety_initialized() { head.next = nullptr; }
 ...
 public:// setter
  ...
  iterator previous(const_iterator pos) {
    return iterator(
        reinterpret_cast<list_node *>(slist_previous(&head, pos.node)));
  }
 ...
 private:// aux interface for insert
  ...
  void insert_after_fill(list_node_base *pos, size_type n,
 ...
 public:// swap
  void swap(slist &rhs) noexcept { MiniSTL::swap(head.next, rhs.head.next); }
 ...
 public:// reverse
  void reverse() {
    if (head.next) head.next = slist_reverse(head.next);
  }

 public:// merge
  void merge(
      slist &);// merge two sorted slists and make the new slist sorted too
...
slist<T, Alloc> &slist<T, Alloc>::operator=(const slist &rhs) {
  if (this != &rhs) {
    ...
    const list_node *n2 =
        reinterpret_cast<const list_node *>(rhs.head.next);
    while (n1 && n2) {
      n1->data = n2->data;
      p1 = n1;
    ...
    if (n2 == nullptr)
      ...
      insert_after_range(p1, const_iterator(const_cast<list_node *>(n2)),
