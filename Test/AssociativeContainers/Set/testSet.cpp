...
TEST_F(SetTest, SWAP) {
    ...
    si.swap(temp_si);
    int i = 1;
    for (auto e : temp_si) EXPECT_EQ(e, i++);
    for (auto e : si) EXPECT_EQ(e, i++);
}
...
TEST_F(SetTest, BEGIN_AND_END) {
    ...
    i = 1;
    for (auto it = si.cbegin(); it != si.cend(); ++it, ++i) EXPECT_EQ(*it, i);
    i = 5;
    for (auto it = si.rbegin(); it != si.rend(); ++it, --i) EXPECT_EQ(*it, i);
    i = 5;
    for (auto it = si.crbegin(); it != si.crend(); ++it, --i) EXPECT_EQ(*it, i);
    set<int, greater<int>> sgi{3, 2, 1, 4, 5};
    int j = 5;
    for (auto it = sgi.begin(); it != sgi.end(); ++it, --j) EXPECT_EQ(*it, j);
    j = 5;
    for (auto it = sgi.cbegin(); it != sgi.cend(); ++it, --j) EXPECT_EQ(*it, j);
    j = 1;
    for (auto it = sgi.rbegin(); it != sgi.rend(); ++it, ++j) EXPECT_EQ(*it, j);
    j = 1;
