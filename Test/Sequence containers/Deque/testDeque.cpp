...
TEST_F(DequeTest,BEGIN_AND_END){
    for(auto it = di.begin();it != di.end();++it){ EXPECT_EQ(*it, it - di.begin()+1); *it = 5; }
    for(auto it = di.cbegin();it != di.cend();++it) EXPECT_EQ(*it,5);
    for(auto it = di.rbegin();it != di.rend();++it){ EXPECT_EQ(*it,5); *it = 3; }
    for(auto it = di.crbegin();it != di.crend();++it){ EXPECT_EQ(*it,3); }
    for(auto it = df.begin();it != df.end();++it){ EXPECT_EQ(*it, it - df.begin()+1.0f); *it = 5.0f; }
    for(auto it = df.cbegin();it != df.cend();++it) EXPECT_EQ(*it,5.0f);
    for(auto it = df.rbegin();it != df.rend();++it){ EXPECT_EQ(*it,5.0f); *it = 3.0f; }
    for(auto it = df.crbegin();it != df.crend();++it){ EXPECT_EQ(*it,3.0f); }
    for(auto it = dd.begin();it != dd.end();++it){ EXPECT_EQ(*it, it - dd.begin()+1.0f); *it = 5.0f; }
    for(auto it = dd.cbegin();it != dd.cend();++it) EXPECT_EQ(*it,5.0f);
    for(auto it = dd.rbegin();it != dd.rend();++it){ EXPECT_EQ(*it,5.0f); *it = 3.0f; }
    for(auto it = dd.crbegin();it != dd.crend();++it){ EXPECT_EQ(*it,3.0f); }
    for(auto it = ds.begin();it != ds.end();++it){ EXPECT_EQ(*it, "hello"); *it = "world"; }
    for(auto it = ds.cbegin();it != ds.cend();++it) EXPECT_EQ(*it,"world");
    for(auto it = ds.rbegin();it != ds.rend();++it){ EXPECT_EQ(*it,"world"); *it = "hello"; }
    for(auto it = ds.crbegin();it != ds.crend();++it){ EXPECT_EQ(*it,"hello"); }
    for(auto it = ddi.begin();it != ddi.end();++it){ EXPECT_EQ(*it, deque<int>({1,2,3})); *it = {4,5,6}; }
    for(auto it = ddi.cbegin();it != ddi.cend();++it) EXPECT_EQ(*it,deque<int>({4,5,6}));
    for(auto it = ddi.rbegin();it != ddi.rend();++it){ EXPECT_EQ(*it,deque<int>({4,5,6})); *it = {7,8,9}; }
    for(auto it = ddi.crbegin();it != ddi.crend();++it){ EXPECT_EQ(*it,deque<int>({7,8,9})); }
}
...
TEST_F(DequeTest,RANDOMACCESS){
    ...
    for(size_t i = 0;i != ddi.size(); ++i) EXPECT_EQ(ddi[i],deque<int>({1,2,3}));
}
