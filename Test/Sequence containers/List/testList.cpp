...
TEST_F(ListTest,BEGIN_AND_END){
    for(auto it = li.begin();it != li.end();++it){ *it = 5; }
    for(auto it = li.cbegin();it != li.cend();++it) EXPECT_EQ(*it,5);
    for(auto it = li.rbegin();it != li.rend();++it){ EXPECT_EQ(*it,5); *it = 3; }
    for(auto it = li.crbegin();it != li.crend();++it){ EXPECT_EQ(*it,3); }
    for(auto it = lf.begin();it != lf.end();++it){ *it = 5.0f; }
    for(auto it = lf.cbegin();it != lf.cend();++it) EXPECT_EQ(*it,5.0f);
    for(auto it = lf.rbegin();it != lf.rend();++it){ EXPECT_EQ(*it,5.0f); *it = 3.0f; }
    for(auto it = lf.crbegin();it != lf.crend();++it){ EXPECT_EQ(*it,3.0f); }
    for(auto it = ld.begin();it != ld.end();++it){ *it = 5.0f; }
    for(auto it = ld.cbegin();it != ld.cend();++it) EXPECT_EQ(*it,5.0f);
    for(auto it = ld.rbegin();it != ld.rend();++it){ EXPECT_EQ(*it,5.0f); *it = 3.0f; }
    for(auto it = ld.crbegin();it != ld.crend();++it){ EXPECT_EQ(*it,3.0f); }
    for(auto it = ls.begin();it != ls.end();++it){ EXPECT_EQ(*it, "hello"); *it = "world"; }
    for(auto it = ls.cbegin();it != ls.cend();++it) EXPECT_EQ(*it,"world");
    for(auto it = ls.rbegin();it != ls.rend();++it){ EXPECT_EQ(*it,"world"); *it = "hello"; }
    for(auto it = ls.crbegin();it != ls.crend();++it){ EXPECT_EQ(*it,"hello"); }
    for(auto it = lli.begin();it != lli.end();++it){ EXPECT_EQ(*it, list<int>({1,2,3})); *it = {4,5,6}; }
    for(auto it = lli.cbegin();it != lli.cend();++it) EXPECT_EQ(*it,list<int>({4,5,6}));
    for(auto it = lli.rbegin();it != lli.rend();++it){ EXPECT_EQ(*it,list<int>({4,5,6})); *it = {7,8,9}; }
    for(auto it = lli.crbegin();it != lli.crend();++it){ EXPECT_EQ(*it,list<int>({7,8,9})); }
}
