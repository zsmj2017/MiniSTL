...
TEST_F(SlistTest, MOVEASSIGN_WITH_SELF) {
    ...
    for (auto li : lli)
        EXPECT_EQ(li, slist<int>({1, 2, 3}));
    for (auto li : temp_lli)
        EXPECT_NE(li, slist<int>({1, 2, 3}));
}
...
TEST_F(SlistTest, BEGIN_AND_END) {
    ...
    for (auto it = lf.cbegin(); it != lf.cend(); ++it)
        EXPECT_EQ(*it, 5.0f);
    for (auto it = ld.begin(); it != ld.end(); ++it) {
        *it = 5.0f;
    }
    for (auto it = ld.cbegin(); it != ld.cend(); ++it)
        EXPECT_EQ(*it, 5.0f);
...
TEST_F(SlistTest, SPLICE) {
    ...
    int i = 1;
    for (auto it = li.cbegin(); it != li.cend(); ++it, ++i)
        EXPECT_EQ(*it, i);
