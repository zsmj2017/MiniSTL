...
TEST_F(SlistTest,BEGIN_AND_END){
    ...
    for(auto it = lli.begin();it != lli.end();++it){ EXPECT_EQ(*it, slist<int>({1,2,3})); *it = {4,5,6}; }
    for(auto it = lli.cbegin();it != lli.cend();++it) EXPECT_EQ(*it,slist<int>({4,5,6}));
}
...
TEST_F(SlistTest,PUSH_AND_POP){
    ...
    EXPECT_EQ(lli.front(),slist<int>({0,0,0})); EXPECT_EQ(lli.size(),3);
}
...
TEST_F(SlistTest,SPLICE){
    ...
    EXPECT_EQ(ls.size(),4);EXPECT_EQ(temp_ls.size(),0);
}
