...
namespace MiniSTL {
    ...
    // temporary_buffer
    template <class T> pair<T, ptrdiff_t> __get_temporary_buffer(ptrdiff_t len, T *) {
    ...
    template <class T> inline pair<T, ptrdiff_t> get_temporary_buffer(ptrdiff_t len) {
        return __get_temporary_buffer(len, reinterpret_cast<T *>(nullptr));
    }

    template <class T> inline pair<T, ptrdiff_t> get_temporary_buffer(ptrdiff_t len, T *) {
        return __get_temporary_buffer(len, reinterpret_cast<T *>(nullptr));
    }
    ...
    template <class ForwardIterator, class T> class __temporary_buffer {
    private: // data member
    ...
    private: // allocate && init
        void allocate_buffer() {
            ...
            if (len > static_cast<ptrdiff_t>(INTPTR_MAX / sizeof(T)))
                len = INTPTR_MAX / sizeof(T);
            while (len > 0) {
                buffer = reinterpret_cast<T *>(malloc(static_cast<size_t>(len) * sizeof(T)));
                if (buffer)
                    break;
                len /= 2;
            }
    ...
    template <class ForwardIterator, class T> struct temporary_buffer : public __temporary_buffer<ForwardIterator, T> {
